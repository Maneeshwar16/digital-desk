...
const Card = ({ data, onAdd, onDelete, onMarkDone, reference, highlight }) => {
  ...
  return (
    ...
    >
      ...
      {/* Task name */}
      <p className="text-white text-xl font-bold mb-1">{data.taskName}</p>

      {/* ✅ Show username if available */}
      {data.userName && (
        <p className="text-gray-400 text-sm mb-3">By {data.userName}</p>
      )}

      {/* Task description rendering based on type */}
      {data.type === 'youtube' || data.type === 'article' ? (
        <>
          {data.taskDescription && isValidUrl(data.taskDescription) ? (
            <a 
        ...
        <div className="mb-6">
          <p className="text-white text-sm mb-2">{data.taskDescription}</p>
          <img 
